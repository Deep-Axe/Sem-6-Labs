/*
 * Token Ring Protocol - Client 2
 *
 * Client 2 is an intermediate node. It receives the token from Client 1
 * (its only predecessor in the ring), holds it for 40 seconds, and then
 * forwards it to Client 3 (its only successor). If Client 3 is down,
...
int main(int argc, char *argv[])
{
    ...
    printf("=============================================\n");
    printf("  Token Ring Protocol — Client 2 (Relay)\n");
    printf("=============================================\n");
    ...
    printf("[Client 2] Waiting for token from Client 1 (only neighbor)...\n\n");
    ...
    if (connect(sockfd, (struct sockaddr *)&dest_addr, sizeof(dest_addr)) < 0) {
        /*
         * TOKEN RING: No bypass! Client 2 can only send to Client 3.
         * If Client 3 is down, the ring is broken.
         */
        printf("[Client 2] Client 3 is DISCONNECTED.\n");
        printf("[Client 2] RING IS BROKEN — token cannot be forwarded.\n");
        printf("[Client 2] Token LOST.\n");
        close(sockfd);
        return EXIT_FAILURE;
    }

...
int main() {
    ...
    printf("[Client1] Connected to server.\n");
    printf("[Client1] Local socket address: %s:%d\n", local_ip, local_port);
    ...
    if (bytes_read > 0) {
        ...
        if (strcmp(buffer, "terminate session") == 0) {
            printf("[Client1] Received TERMINATE from server. Closing.\n");
        } else {
            printf("[Client1] Server response: \"%s\"\n", buffer);
        }
    } else {
        printf("[Client1] Server disconnected.\n");
    }

    close(sock_fd);
    printf("[Client1] Connection closed.\n");
